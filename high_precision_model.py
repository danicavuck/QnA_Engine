...
def train_lstm(return_value):
    ...
    malstm_distance = Lambda(function=lambda x: K.exp(-K.sum(K.abs(x[0] - x[1]), axis=1, keepdims=True)),
    ...
    malstm.save('serialized/model.h5')

    return malstm,max_seq_length
...
questions = joblib.load('serialized/top100_questions')
...
#malstm,max_seq_length = train_lstm(return_value)

malstm = keras.models.load_model('serialized/model.h5')
